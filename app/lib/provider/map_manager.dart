...
class _StoredOccupancyMap {
  const _StoredOccupancyMap({
    required this.width,
...
String _encodeStoredOccupancyMap(_StoredOccupancyMap map) {
...
_StoredOccupancyMap _decodeStoredOccupancyMap(String source) {
  final json = jsonDecode(source) as Map<String, dynamic>;
  final dataList = json['data'] as List;
  return _StoredOccupancyMap(
...
class MapManager extends ChangeNotifier {
  ...
  ValueNotifier<OccupancyMap> occupancyMap = ValueNotifier<OccupancyMap>(
    OccupancyMap(),
  );
  ValueNotifier<TopologyMap> topologyMap = ValueNotifier<TopologyMap>(
    TopologyMap(points: []),
  );

  Future<void> init() {
    return _initFuture ??= loadLocalOccupancyMap();
  }
  ...
  void addNavPoint(NavPoint point) {
    final index = topologyMap.value.points.indexWhere(
      (p) => p.name == point.name,
    );
    if (index != -1) {
  ...
  void removeNavPoint(String name) {
    ...
    topologyMap.value.routes.removeWhere(
      (r) => r.fromPoint == name || r.toPoint == name,
    );
    topologyMap.notifyListeners();
    notifyListeners();
  }
  ...
  void updateTopologyMapFromRos(TopologyMap rosTopologyMap) {
    ...
    notifyListeners();
    print(
      'MapManager: 收到ROS拓扑地图，${rosTopologyMap.points.length}个点，${rosTopologyMap.routes.length}条路径',
    );
  }
  ...
  Future<void> saveLocalOccupancyMap() async {
    ...
    await prefs.setString(_occupancyMapKey, encoded);
  ...
  Future<void> loadLocalOccupancyMap() async {
    ...
    if (jsonStr != null) {
      try {
        final stored = await compute(_decodeStoredOccupancyMap, jsonStr);
        final map = OccupancyMap();
        map.mapConfig.width = stored.width;
        map.mapConfig.height = stored.height;
        map.mapConfig.resolution = stored.resolution;
        map.mapConfig.originX = stored.originX;
        map.mapConfig.originY = stored.originY;
        map.data = stored.data;

        occupancyMap.value = map;
        occupancyMap.notifyListeners();
        print('MapManager: 从本地加载栅格地图');
      } catch (error) {
        print('MapManager: 加载本地栅格地图失败: $error');
      }
    }
  }
